...
class SplitRegion:
    ...
    def findTurnPoints(self):
        ...
        for poly in self.polygons:
            for  i in range(poly.count()-1):
                pts = poly.points
                v1 = pts[-2if (i==0)else(i-1)].pointTo(pts[i])
    ...
    def split(self):
        ...
        for pt in turnPts:
            ys.append(pt.y)
        ys.sort()
        hatchPtses = calHatchPoints(self.polygons,ys)
        splitters = []
        ...
        if len(splitters) != 0:
            ...
            clipper.AddPaths(ca.toPaths(self.polygons),PT_SUBJECT,True)
            clipper.AddPaths(ca.toPaths(splitters),PT_CLIP,True)
